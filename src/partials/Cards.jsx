        ...
        <div className="p-[2.37%] cards flex flex-wrap bg-[#18181b] w-full gap-x-11">
            {data.map((card, index) => (
                <Link key={index} className="relative w-[27.3vh] my-3">
                    <img className="shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)] h-[40vh] w-full object-cover rounded" src={(card.poster_path||card.backdrop_path || card.profile_path)?`https://image.tmdb.org/t/p/original/${card.poster_path || card.backdrop_path || card.profile_path}`: noImg} alt="img"/>
